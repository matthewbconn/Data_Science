...
dim(X);
keepX <-X[,1]; # use this to keep the actual value of the image 
               # (this will be a vector of 3's)

X<-X[,-1]; # we are excluding the digit ID (for plotting) since all == 3
dim(X)     # now size is [total samples] x [256]

myPhi = X; # use this to get a feature map which is simply the identity
print(dim(myPhi)) # confirms that we have 658 complete samples to form our shit with
...
for (i in 1:10) {
  W <- ridgeReg(myPhi, keepX,0.1); # arbitrary lambda = 0.1
  # plot(myPhi[i,]%*%W)# plot one sample (one image's pixel values applied to w_lam)
}

lamVal <- 0.1; # arbitrary lambda value
noiseVal <- 0.5;
W<-ridgeReg(myPhi, keepX, lamVal);
for (i in 1:10) {
  zimage1((Xave+noiseVal*rnorm(Sig)))
}
